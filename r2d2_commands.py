...
class Robot:
    ...
    def createSentenceEmbeddings(self):
        trainingSentences = self.bert.createSentenceEmbeddings("data/r2d2TrainingSentences.txt")

        self.categories = [x for x in trainingSentences]

    def inputCommand(self, command):
        commandType = self.bert.getCategory(command)
    ...
    def lightParser(self, command):
        ...
        # intensities - keeps of what intensity to change those lights to, combination of blink, dim, off, on
        ...
        if "holoemitter" in command or "holo emitter" in command:
            slots["lights"].append("holoEmit")
        if "logic display" in command:
            slots["lights"].append("logDisp")
        ...
        for word in words:
            ...
            front = self.bert.contextWordSim("front", word)
            if back > self.wordSimCutoff and back > front:
            ...
            if on > self.wordSimCutoff and on > off:
                slots["intensities"].add("on")
        ...
        if "subtract" in command:
            slots["valueChange"] = "sub" # subtract gets split into three words in bert, using the mean makes it less similar to decrease than add
